...
class GFAuthorizeNet {
    ...
    public static function init(){
        ...
        if(is_admin()){
            ...
            if(self::is_authorizenet_page()){
                ...
                add_filter("gform_currency_setting_message", create_function("","echo '<div class=\'gform_currency_message\'>Authorize.Net only supports Canadian Dollars, US Dollars, Pound Sterling, and Euro.</div>';"));
            }
    ...
    private static function authorize_credit_card($form_data, $config, $validation_result){
        ...
        if($auth_response->approved ){
						$gAuthorizePaymentAmount = $form_data["amount"];
    ...
    private static function create_subscription($config, $entry, $form){
        ...
        if(!empty($fee_amount) && $fee_amount > 0){
                ...
                self::log_debug("Capturing setup fee payment.");
